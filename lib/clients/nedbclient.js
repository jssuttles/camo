...
var createCollection = function(collectionName, url, options, errCB) {
	options = options || {};
	if (url === 'memory') {
		Object.assign(options, {inMemoryOnly: true});
		return new Datastore({inMemoryOnly: true});
	}
	var collectionPath = getCollectionPath(url, collectionName);
	Object.assign(options, {filename: collectionPath, autoload: true, onload: function(err) {
		if (err) {
			errCB(err);
		}
	}});
...
var getCollection = function(name, collections, path, options, errCB) {
	if (!(name in collections)) {
		var collection = createCollection(name, path, options, errCB);
...
class NeDbClient extends DatabaseClient {
	...
	save(collection, id, values) {
		...
		return new Promise(function(resolve, reject) {
			var db = getCollection(collection, that._collections, that._path, that._options, reject);
    ...
    createIndex(collection, field, options) {
        ...
        options.sparse = options.sparse || false;
				return new Promise(function(resolve, reject) {
	        var db = getCollection(collection, this._collections, this._path, this._options, reject);
	        db.ensureIndex({fieldName: field, unique: options.unique, sparse: options.sparse}, function(error) {
						if (error) return reject(error);
	    			return resolve();
					});
				});
